...
//    }
    ...
    public TextureAtlas.Preparations prepareToStitch(ResourceManager resourceManagerIn, Stream<ResourceLocation> resourceLocationsIn, ProfilerFiller profilerIn, int maxMipmapLevelIn) {
        ...
        for (TextureAtlasSprite.Info spriteInfo : this.getBasicSpriteInfos(resourceManagerIn, set)) {
        ...
        List<TextureAtlasSprite> list = this.getLoadedSprites(resourceManagerIn, stitcher, k1);
        profilerIn.pop();
        return new TextureAtlas.Preparations(set, stitcher.getWidth(), stitcher.getHeight(), k1, list);
    }

    private Collection<TextureAtlasSprite.Info> getBasicSpriteInfos(ResourceManager resourceManagerIn, Set<ResourceLocation> spriteLocationsIn) {
        ...
        for (ResourceLocation thickSpriteLocation : spriteLocationsIn) {
            if (!MissingTextureAtlasSprite.getLocation().equals(thickSpriteLocation)) {
                list.add(CompletableFuture.runAsync(() -> {
                    ...
                    ResourceLocation baseSpritePath = this.getResourceLocation(baseSpriteLocation);
    ...
    private List<TextureAtlasSprite> getLoadedSprites(ResourceManager resourceManagerIn, Stitcher stitcherIn, int mipmapLevelIn) {
        ...
        stitcherIn.gatherSprites((spriteInfo, width, height, x, y) -> {
            if (spriteInfo == MissingTextureAtlasSprite.info()) {
                ...
                list.add(CompletableFuture.runAsync(() -> {
                    TextureAtlasSprite textureatlassprite = this.load(resourceManagerIn, spriteInfo, width, height, mipmapLevelIn, x, y);
    ...
    @Nullable
    private TextureAtlasSprite load(ResourceManager resourceManagerIn, TextureAtlasSprite.Info spriteInfoIn, int widthIn, int heightIn, int mipmapLevelIn, int originX, int originY) {
        ResourceLocation baseSpritePath = this.getResourceLocation(spriteInfoIn.name());
