...
public abstract class BranchBlock extends BlockWithDynamicHardness implements TreePart, FutureBreakable {
    ...
    /**
     * Strips the {@link BranchBlock}. This should only be called if {@link Family#hasStrippedBranch()} evaluates to
     * {@code true}.
     *
     ...
     * @param player   The {@link Player} stripping the branch.
    ...
    /**
     * Checks if the block is path-findable. This prevents mobs like bees from getting stuck in branches.
     *
     * @param state   The {@link BlockState} for the {@link BranchBlock}.
     * @param worldIn The {@link BlockGetter} instance.
     * @param pos     The {@link BlockPos} of the {@link BranchBlock}.
     * @param type    The {@link PathComputationType} to check.
    ...
    /**
     * We override the standard behavior because we need to preserve the tree network structure to calculate the wood
     * volume for drops. {@link super#removedByPlayer(BlockState, World, BlockPos, PlayerEntity, boolean, FluidState)}
     ...
     * @param player      The {@link Player} damaging the {@link Block}, or {@code null}.
     * @param willHarvest {@code true} if {@link #playerDestroy(World, PlayerEntity, BlockPos, BlockState, TileEntity,
     *                    ItemStack)} will be called after this, if the return is {@code true}.
     * @param fluid       The current {@link FluidState} for the position in the {@link Level}.
     ...
     */
    public void breakDeliberate(LevelAccessor world, BlockPos pos, DynamicTrees.DestroyMode mode) {
        destroyMode = mode;
